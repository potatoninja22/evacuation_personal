...
class Water(FloorObject):
    ...
    def step(self):
        ...
        for agent in contents:
            if isinstance(agent,Elevation):
                height = agent.elevation
        ...
        for cell in neighborhood:
            ...
            for agent in cont:
                if isinstance(agent,Elevation):
                    if agent.elevation < height and flag == False:
                        water = Water(self.model,(cell[0],cell[1]))
                        water.unique_id = self.model.next_id()
...
class Human(Agent):
    ...
    def get_visible_tiles(self) -> tuple[Coordinate, tuple[Agent]]:
        ...
        # Using set here to avoid repetition and increase efficiency
    ...
    def panic_rules(self):
        ...
        if not self.believes_alarm and shock_modifier != self.DEFAULT_SHOCK_MODIFIER:
            print("Agent now believes the flood is real!")
                ...
                if not self.model.grid.out_of_bounds(retreat_location):
                    ...
                    for agent in contents:
                        if isinstance(agent, Water):
                            ...
                            print("Agent surrounded by water and moving randomly")
                            retreat_location = None
                            break

                    if retreat_location:
                        print("Agent retreating opposite to water")
